...
func (h hand) Type() handType {
	...
	if numberOfJokers, ok := occuredCards["J"]; ok {
		maxCardOccurence := cardOccurence{}
		for k, v := range occuredCards {
			if k == "J" {
				continue
			}
			if v > maxCardOccurence.occurence {
				maxCardOccurence = cardOccurence{
					card:      k,
					occurence: v,
				}
		...
		occuredCards[maxCardOccurence.card] += numberOfJokers
		delete(occuredCards, "J")
	}
